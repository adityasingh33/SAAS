import { useRef, useEffect, useState, version } from 'react';
...
const Sidebar = ({ isMenuOpen, project, setProject, isGenerating, setIsGenerating }: SidebarProps) => {
  ...
  return (
    <div className={`h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800 tarnsition-all ${isMenuOpen ? 'max-sm:w-0 overflow-hidden' : 'w-full'}`}>
      <div className='  flex flex-col h-full'>
            ...
            if (isMessage) {
              ...
              return (
                <div key={ver.id} className='w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                  <div className='text-xs ont-medium'>
                    Code updated <br />
                    <span>
        ...
        <form onSubmit={handleRevisions} className='m-3 relative'>
          <div className='flex items-center gap-2'>
            ...
            className='absolute bottom-2.5 right-1 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors disabled:opacity-60'>
