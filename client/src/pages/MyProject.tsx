            ...
            <div className='px-4 md:px-16 lg:px-24 xl:px-32'>
                {loading ? (
                        ...
                        <div className='flex flex-wrap gap-3.5'>
                            {projects.map((project) => (
                                <div onClick={() => navigate(`/projects/${project.id}`)} key={project.id}
                                     className='relative group w-72 max-sm:mx-auto cursor-pointer rounded-lg overflow-hidden bg-gray-900/60 border border-gray-700 shadow-md group-hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duratiom-300'>
                                    ...
                                    {/*content*/}
                                    <div
                                        className='p-4 text-white bg-linear-180 from transparent group-hover:from-indigo-950 to-transparent tarnsition-colors '>
                                        ...
                                        <div className='flex flex-col gap-3 items-center'>
                                            ...
                                            <span
                                                className='text-xs text-gray-500'>{new Date(project.createdAt).toLocaleDateString()}</span>
                                            </div>

                                            <div className='flex flex-row gap-3 text-white text-sm '>
                                                <button onClick={() => navigate(`/preview/${project.id}`)}
                                                        className='px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded-md transition-all'>Preview
                                                </button>

                                                <button onClick={() => navigate(`/projects/${project.id}`)}
                                                        className='px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded-md transition-all'>Open
                                                </button>
                                            </div>
                                    ...
                                    <div onClick={e => e.stopPropagation()}>
                                        <TrashIcon className='absolute top-3 scale-0 group-hover:scale-100 bg-white p-1.5 size-7
                    ...
                    <div className='flex flex-col items-center justify-center h-[80vh]'>
                        ...
                        <button onClick={() => navigate('/')}
                                className='text-white px-5 py-2 mt-5 rounded-md bg-indigo-500 hover:bg-indigo-600 '>Create
                            New
                        </button>
                    </div>
                )}
